...
function formatearTexto(t){
  return esc(t).trim().split(/\n{2,}/).map(bloque => {
    if(/^\s*&gt;/.test(bloque)){
      return '<blockquote>' + enfasis(bloque.replace(/^\s*&gt;\s?/gm,'')).replace(/\n/g,'<br>') + '</blockquote>';
    }
...
function buscarComentario(lista, id){
  for(const c of lista){
    if(c.id === id) return c;
    const hallado = buscarComentario(c.respuestas || [], id);
...
function pintarDestacado(){
  ...
  $('#debate-dia').innerHTML = `
    <div class="destacado__linea">
      <span class="destacado__marca"><span class="pulso"></span>Debate del día</span>
      <span class="categoria">${esc(h.categoria)}</span>
      <span class="meta">${haceRato(h.created_at)}</span>
    </div>
...
function miniEncuesta(h){
  ...
  const segmentos = (h.opciones || []).map((o,i) =>
    `<span class="mini__seg" style="width:${total ? (o.votos/total*100).toFixed(1) : 0}%;background:${TINTES[i % TINTES.length]}"></span>`).join('');
  const leyenda = (h.opciones || []).map((o,i) => `
    <span class="mini__item">
...
function tarjeta(h){
  ...
  <article class="tarjeta">
    <div class="tarjeta__cabecera">
      <span class="categoria">${esc(h.categoria)}</span>
      <span class="meta">${haceRato(h.created_at)} · ${esc(h.autor)}</span>
    </div>
    <h2 class="tarjeta__titulo"><a href="#${h.id}" data-ir="${h.id}">${esc(h.titulo)}</a></h2>
    <p class="tarjeta__previa">${esc(h.cuerpo[0])}</p>
    <p class="tarjeta__pregunta">${esc(h.pregunta)}</p>
    ${miniEncuesta(h)}
...
function historiasVisibles(){
  ...
  const ordenes = {
    tendencias: (a,b) => ((respuestas(b)*60 + totalVotos(b)) / (minAprox(b)+120)) - ((respuestas(a)*60 + totalVotos(a)) / (minAprox(a)+120)),
    debatidas:  (a,b) => respuestas(b) - respuestas(a),
    nuevas:     (a,b) => new Date(b.created_at) - new Date(a.created_at),
    votadas:    (a,b) => totalVotos(b) - totalVotos(a)
  };
...
function abrirDebate(id){
  ...
  if(!h) return;
  estado.actual = id;
  estado.ordenCom = 'relevancia';
  $('#vista-feed').hidden = true;
  $('#vista-debate').hidden = false;
  pintarDebate();
...
function pintarEncuesta(h){
  ...
  if(!mio){
    ...
    cuerpo = `<div class="opciones">` + h.opciones.map(o => {
        const pct = total ? (o.votos / total * 100) : 0;
        const esMio = o.id === mio;
      ...
      <div class="encuesta__pie">
        <span class="meta">${plural(total,'voto','votos')}${opcionMia ? (opcionMia.votos === 1 ? ' · nadie más ha votado lo mismo que tú' : ' · ' + num(opcionMia.votos - 1) + ' han votado lo mismo que tú') : ''}</span>
        <span class="meta">Un voto por cuenta</span>
      </div>`;
  }

  $('#modulo-encuesta').innerHTML = `
    <div class="encuesta__cabecera">
      <h3 id="titulo-encuesta">${mio ? 'Así ha votado la gente' : 'Vota antes de leer a los demás'}</h3>
...
function pintarEditor(h){
  ...
  $('#modulo-editor').innerHTML = `
    ...
    <div class="editor__barra" role="toolbar" aria-label="Formato del texto">
      ...
      <button class="herramienta" data-formato="cita" title="Citar">&ldquo; &rdquo;</button>
...
function pintarComentario(c, h){
  ...
  return `
  <div class="comentario" id="${c.id}">
    <div class="comentario__cabecera">
      <span class="avatar" style="width:26px;height:26px;font-size:11px">${esc(c.autor.slice(0,2).toUpperCase())}</span>
      <span class="comentario__autor">${esc(c.autor)}</span>
      ${opcion ? `<span class="insignia-voto">votó <b>${esc(opcion.texto.length>24 ? opcion.texto.slice(0,24)+'…' : opcion.texto)}</b></span>` : ''}
      <span class="meta">${haceRato(c.created_at)}</span>
    </div>
    <div class="comentario__cuerpo">${formatearTexto(c.texto)}</div>
    <div class="comentario__acciones">
      <span class="votos">
        <button data-voto-com="${c.id}" data-dir="1" aria-pressed="${miV===1}" aria-label="A favor">
          <svg width="14" height="14" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 3.2 13 9H3Z" fill="${miV===1?'currentColor':'none'}" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg>
        </button>
        <span class="votos__conteo">${c.arriba - c.abajo}</span>
        <button class="abajo" data-voto-com="${c.id}" data-dir="-1" aria-pressed="${miV===-1}" aria-label="En contra">
          <svg width="14" height="14" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 12.8 3 7h10Z" fill="${miV===-1?'currentColor':'none'}" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/></svg>
        </button>
      </span>
    ...
    <div class="caja-respuesta" data-caja="${c.id}" hidden>
      ...
      <textarea id="resp-${c.id}" placeholder="Responde a ${esc(c.autor)}…"></textarea>
      <div class="caja-respuesta__pie">
        <button class="btn btn--linea btn--pequeno" data-cancelar-respuesta="${c.id}">Cancelar</button>
        <button class="btn btn--principal btn--pequeno" data-enviar-respuesta="${c.id}">Responder</button>
      </div>
    </div>
...
function ordenarComentarios(lista){
  ...
  return estado.ordenCom === 'nuevos'
    ? copia.sort((a,b) => new Date(b.created_at) - new Date(a.created_at))
    : copia.sort((a,b) => (b.arriba - b.abajo) - (a.arriba - a.abajo));
}
...
function pintarLateralDebate(h){
  const total = totalVotos(h);
  $('#panel-contexto').innerHTML = `
    <h3>Sobre esta historia</h3>
    <ul class="lista-tematicas">
      <li><button type="button" style="cursor:default">Publicada <span>${haceRato(h.created_at)}</span></button></li>
      <li><button type="button" style="cursor:default">Votos <span>${num(total)}</span></button></li>
      <li><button type="button" style="cursor:default">Respuestas <span>${contarComentarios(h.comentarios)}</span></button></li>
...
async function votarComentario(idCom, dir){
  const h = estado.historias.find(x => x.id === estado.actual);
  const c = buscarComentario(h.comentarios, idCom);
  if(!c) return;
...
function reportar(id){
  ...
  estado.reportes.push(id);
  guardarLocal();
  avisar('Reportado al equipo de moderación.');
...
function cerrarModal(){
  if(!veloActivo) return;
  veloActivo.hidden = true;
